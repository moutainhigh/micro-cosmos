 ...
 * @Description: 排他网关两条分歧线，其中一条为默认线  测试
 */
@Service
public class ExclusiveGateWayDefaultLineService extends BaseService {
...
//        process.addFlowElement(ActivitiUtils.createUserTask("ut3","ut3","ts"));
        ...
        process.addFlowElement(ActivitiUtils.createSequenceFlow(NodeTypeEnum.START.name(), "g1",null));
...
//        process.addFlowElement(ActivitiUtils.createSequenceFlow("l2","g1", "ut3","${regularService.isNumber(sex) && sex == 1}"));// ${sex == 'male'}
        //变量不能以数字开头，遵循java的基本命名法
//        process.addFlowElement(ActivitiUtils.createSequenceFlow("l3","g1", "ut4","${regularService.isNumber(_sex) && _sex == 2}"));// ${sex == 'female'}

        process.addFlowElement(ActivitiUtils.createSequenceFlow("l3","g1", "ut4","${sex != ''.concat(2)}"));
